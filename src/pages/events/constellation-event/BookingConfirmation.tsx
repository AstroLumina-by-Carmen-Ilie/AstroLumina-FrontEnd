  ...
  useEffect(() => {
    const confirmBooking = async () => {
      try {
        ...
        const validHolders = ticketHolders.filter(
          (h) => h.fullName.trim()
        );
        ...
        setEmailError("Notificările nu au putut fi trimise, dar rezervarea a fost confirmată.");
      } finally {
    ...
    <div className="container px-6 pt-24 pb-16 mx-auto max-w-2xl">
      <div className="p-8 text-center glass-card">
        ...
        <p className="mb-6 text-cosmic-300">
          {ticketCount} bilet(e) rezervat(e) pentru {event?.title || "Constelații"}
        </p>
        ...
        <div className="p-4 mb-8 text-left rounded-lg bg-white/5">
          ...
          <ul className="space-y-2 text-sm text-cosmic-300">
            ...
            <li>Total: {ticketCount * (event?.price || 120)} RON</li>
