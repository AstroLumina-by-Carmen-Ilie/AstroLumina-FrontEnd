...
const ConfirmationStep: React.FC<ConfirmationStepProps> = ({
  ...
  const [bookingConfirmation, setBookingConfirmation] = useState<BookingResponse['booking'] | null>(null);
  ...
  useEffect(() => {
    const createBooking = async () => {
      try {
        ...
        const bookingResponse = await axios.post<BookingResponse>(
          `${BOOKING_API_URL}/api/bookings`,
          {
            ...
            bookingFieldsResponses: {
              ...
              'birth-place': `${payload.city}, ${payload.nation}`,
        ...
        // Send confirmation email (if endpoint exists)
        // try {
        //   await axios.post(`${BOOKING_API_URL}/api/bookings/confirm-email`, {
        //     bookingUid: bookingResponse.data.booking.uid,
        //     email: contactInfo.email,
    ...
    createBooking();
  }, [payload, userInfo, contactInfo, bookingQuestions, selectedSlot, paymentIntentId, BOOKING_API_URL]);
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-center py-12">
          <div className="text-center">
            ...
            <p className="text-cosmic-400 text-sm mt-2">Ne conectăm cu sistemul de programare</p>
          </div>
        </div>
      </div>
    );
    ...
    return (
      <div className="space-y-6">
        <div className="bg-red-500/20 border border-red-500/50 rounded-xl p-6">
          <h3 className="text-red-300 font-semibold mb-2">A apărut o problemă</h3>
    ...
    <div className="space-y-6">
      ...
      <div className="bg-emerald-500/20 border border-emerald-500/50 rounded-xl p-6">
        <div className="flex items-center gap-3 mb-2">
          ...
          <h3 className="text-emerald-300 font-semibold">Sesiune programată cu succes!</h3>
        </div>
      ...
      {bookingConfirmation && (
        <div className="bg-white/5 rounded-xl border border-white/10 p-6 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <p className="text-cosmic-400 text-xs font-semibold uppercase mb-1">Tip sesiune</p>
              <p className="text-cosmic-100">Astrograma Natală și Karmică</p>
            </div>
            <div>
              <p className="text-cosmic-400 text-xs font-semibold uppercase mb-1">Data și ora</p>
              <p className="text-cosmic-100">{formatDate(bookingConfirmation.startTime)}</p>
            </div>
            <div>
              <p className="text-cosmic-400 text-xs font-semibold uppercase mb-1">Participant</p>
              <p className="text-cosmic-100">{bookingConfirmation.attendee.name}</p>
            </div>
            <div>
              <p className="text-cosmic-400 text-xs font-semibold uppercase mb-1">Email</p>
              <p className="text-cosmic-100 text-sm break-all">{bookingConfirmation.attendee.email}</p>
            </div>
            <div>
              <p className="text-cosmic-400 text-xs font-semibold uppercase mb-1">Zonă orară</p>
              <p className="text-cosmic-100">{bookingConfirmation.attendee.timeZone}</p>
            </div>
            <div>
              <p className="text-cosmic-400 text-xs font-semibold uppercase mb-1">Status</p>
              <p className="text-emerald-400 capitalize">{bookingConfirmation.status}</p>
            </div>
          </div>

          <div className="pt-4 border-t border-white/10">
            <p className="text-cosmic-400 text-xs font-semibold uppercase mb-2">ID Sesiune</p>
            <p className="text-cosmic-300 font-mono text-xs break-all">{bookingConfirmation.uid}</p>
          </div>
        </div>
      )}
        ...
        <ul className="space-y-3">
          <li className="flex gap-3">
            ...
            <span className="text-cosmic-300 text-sm">
              Vei primi o confirmă pe email cu link-ul Zoom și toate detaliile sesiunii
            </span>
          </li>
          <li className="flex gap-3">
