...
const FinalStep: React.FC<FinalStepProps> = ({ payload, userInfo, contactInfo, result: preCalculatedResult }) => {
  ...
  const calculateResults = async () => {
    ...
    if (preCalculatedResult) {
      ...
      try {
        // Get the full natal chart data including aspects and chart
        const fullResult = await calculateNatalChart('ro', payload);
        setResult(fullResult);
      } catch (error) {
        console.error('Error fetching reading:', error);
        setErrorMessage('A apărut o eroare la calcularea astrogramei. Te rog încearcă din nou.');
        setResult(null);
      } finally {
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      const doc = await generateNatalChartPDF(result.astral_elements, result.astral_chart, userInfo, contactInfo);
          ...
          <div className="space-y-6">
            {chartImageSrc && (
              <div className="rounded-2xl overflow-hidden border border-white/10">
                <img src={chartImageSrc} alt="Astrograma Natală" className="w-full object-cover" />
              </div>
            )}
