...
const ResultsDisplay: React.FC<{
  ...
  const planetsData =
    splitIndex === -1
      ? result.astral_elements
      : result.astral_elements.slice(0, splitIndex);
  const asteroidsData =
    splitIndex === -1 ? [] : result.astral_elements.slice(splitIndex);
  ...
  const handleDownloadPDF = async () => {
    if (result && userInfo) {
      ...
      try {
        ...
        await doc.save(
          `Pozitia_Astrelor_${userInfo.name.replace(/\s+/g, "_")}.pdf`,
        );
      } finally {
  ...
  return (
    <div className="w-full text-center">
      <div className="mb-6">
        <h3 className="mb-2 text-2xl font-bold text-cosmic-100">
          {userInfo.name}
        </h3>
      ...
      <div className="rounded-xl border bg-white/5 border-white/10">
        ...
        <div className="flex px-4 mb-4 border-b border-white/10">
          <button
            type="button"
                ...
                <tr key={index} className="border-b border-white/5">
                  <td className="p-2 text-sm whitespace-normal sm:p-3 text-cosmic-200 sm:text-base">
                    {activeTab === 2 ? (
                      ...
                      <>
                        <span className="mr-2 font-semibold astronomicon-symbol-fallback">
                          {info.symbol || ""}
                        </span>
                        {info.name}
                      </>
